  ...
  return (
    <div className="mt-6 pt-2 border-t border-slate-100 animate-fadeIn origin-top">
                ...
                <div>
                   ...
                   <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {fund.news.map((item, idx) => (
                      <a key={idx} href={item.url} target="_blank" rel="noopener noreferrer" className="block group bg-white p-4 rounded-xl border border-slate-200 hover:border-blue-400 hover:shadow-lg transition-all">
            ...
            <div className="space-y-6 animate-fadeIn">
               <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                 {['ytd', 'month1', 'month3', 'month6', 'year1', 'year3', 'year5', 'volatility3y'].map(key => (
                   <PerformanceMetric key={key} label={t(key as any)} value={fund.performance?.[key as keyof FundPerformance]} />
                 ))}
               </div>
               <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
                  <p className="text-xs text-slate-400 text-center">* {t('performance')} based on NAV return</p>
               </div>
            </div>
          )}
          {activeTab === 'data' && (
              ...
              {fund.dividendInfo ? (
                <>
                   ...
                   {fund.dividendInfo.history && fund.dividendInfo.history.length > 0 ? (
                     ...
                     <div className="text-center py-8 text-slate-400 bg-white rounded-xl border border-dashed border-slate-200">No dividend history available</div>
                   )}
                </>
              ) : (
                <div className="text-center py-8 text-slate-400 bg-white rounded-xl border border-dashed border-slate-200">Accumulating fund or no dividend data</div>
              )}
            </div>
          )}
          {activeTab === 'portfolio' && (
            <div className="space-y-6 animate-fadeIn">
               <div>
                  <h4 className="text-sm font-bold text-slate-800 mb-3 flex items-center gap-2">{t('topHoldings')}</h4>
               ...
               <div>
                  <h4 className="text-sm font-bold text-slate-800 mb-3 flex items-center gap-2">{t('sectorAllocation')}</h4>
             ...
             <div className="animate-fadeIn space-y-3">
                {fund.documents && fund.documents.length > 0 ? (
                  fund.documents.map((doc, i) => (
                    <a key={i} href={doc.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-4 bg-white rounded-xl border border-slate-200 hover:border-blue-400 hover:shadow-md transition-all group">
                       ...
                       <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-slate-300 group-hover:text-blue-500" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M3 17a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm3.293-7.707a1 1 0 011.414 0L9 10.586V3a1 1 0 112 0v7.586l1.293-1.293a1 1 0 111.414 1.414l-3 3a1 1 0 01-1.414 0l-3-3a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
                    </a>
                  ))
                ) : (
                  <div className="text-center py-10">
                     <p className="text-slate-400 text-sm">{t('noDocs')}</p>
                     <a href={`https://www.google.com/search?q=${encodeURIComponent(fund.name + ' Factsheet')}`} target="_blank" rel="noopener noreferrer" className="text-blue-500 text-xs font-bold mt-2 inline-block hover:underline">Search on Google</a>
                  </div>
                )}
             </div>
          )}
       ...
       <div className="flex gap-3 mt-6">
          <button onClick={() => onRefresh(fund.isin)} disabled={isRefreshing} className="flex-1 border py-3 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 bg-white border-slate-200 hover:bg-slate-50 text-slate-700 shadow-sm">
              {isRefreshing ? (
...
const AppContent: React.FC = () => {
  ...
  const getPrefsKey = useCallback(() => user ? `preferences_${user.sub}` : 'preferences', [user]);

  const [funds, setFunds] = useState<Fund[]>([]);
  const [preferences, setPreferences] = useState<AppPreferences>({ 
    showPerformance: true, showChart: true, showDividends: true, compactMode: false 
  });
  ...
  const [tradeModalConfig, setTradeModalConfig] = useState<{ isOpen: boolean; type: 'BUY' | 'SELL'; fundIsin: string | null }>({ isOpen: false, type: 'BUY', fundIsin: null });
  ...
  useEffect(() => {
    const savedFunds = localStorage.getItem(getFundsKey());
    setFunds(savedFunds ? JSON.parse(savedFunds) : []);
    
    const savedPrefs = localStorage.getItem(getPrefsKey());
    if (savedPrefs) setPreferences(JSON.parse(savedPrefs));
    
    setAnalysis(null);
    setSelectedFunds(new Set());
    setExpandedFunds(new Set());
  }, [getFundsKey, getPrefsKey]);

  // Persistence
  useEffect(() => {
    localStorage.setItem(getFundsKey(), JSON.stringify(funds));
  }, [funds, getFundsKey]);

  useEffect(() => {
    localStorage.setItem(getPrefsKey(), JSON.stringify(preferences));
  }, [preferences, getPrefsKey]);
  ...
  const handleAddFund = async (queries: string[]) => {
    ...
    const newQueries = uniqueInput.filter(q => !funds.some(f => f.isin.toLowerCase() === q.toLowerCase() || f.name.toLowerCase() === q.toLowerCase()));
    if (newQueries.length === 0) { showNotification('duplicateFund', 'error'); return; }
    setBackgroundTask({ active: true, total: newQueries.length, completed: 0, success: 0, fail: 0 });
  ...
  const toggleFavorite = (isin: string, e: React.MouseEvent) => {
    e.stopPropagation();
    setFunds(prev => prev.map(f => f.isin === isin ? { ...f, isFavorite: !f.isFavorite } : f));
  };
  ...
  const handleTradeConfirm = (fundIsin: string, type: 'BUY' | 'SELL', quantity: number) => {
    setFunds(prev => prev.map(f => f.isin === fundIsin ? { ...f, unitsHeld: type === 'BUY' ? (f.unitsHeld || 0) + quantity : (f.unitsHeld || 0) - quantity } : f));
  ...
  const displayedFunds = showFavoritesOnly ? funds.filter(f => f.isFavorite) : funds;
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 pb-20 font-sans relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center gap-3">
              {user ? (
                <div className="flex items-center gap-3 pr-2 border-r border-slate-200">
                  <div className="flex flex-col items-end hidden md:flex">
                    ...
                    <span className="text-[10px] text-emerald-500 font-bold uppercase tracking-tighter">Account Synced</span>
                  </div>
                  <div className="relative group">
                    <img src={user.picture} alt={user.name} className="w-9 h-9 rounded-full border-2 border-white shadow-sm cursor-pointer" />
                    <div className="absolute top-full right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-slate-100 opacity-0 group-hover:opacity-100 transition-opacity invisible group-hover:visible pointer-events-none group-hover:pointer-events-auto overflow-hidden">
                       <div className="px-4 py-3 border-b border-slate-50">
                         <p className="text-xs text-slate-400 font-medium">Logged in as</p>
                         <p className="text-sm font-bold text-slate-700 truncate">{user.email}</p>
                       </div>
                       <button onClick={handleLogout} className="w-full text-left px-4 py-3 text-sm text-red-500 font-bold hover:bg-red-50 transition-colors flex items-center gap-2">
              ...
              <button onClick={() => setIsSettingsOpen(true)} className="p-2 text-slate-500 hover:bg-slate-100 rounded-full transition-colors"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg></button>
              <button onClick={() => setLanguage(language === 'zh-TW' ? 'en' : 'zh-TW')} className="px-3 py-1.5 rounded-lg border border-slate-200 text-sm font-medium text-slate-600 hover:bg-slate-50 transition-all active:scale-95">{language === 'zh-TW' ? 'EN' : '中文'}</button>
              <button onClick={() => setIsModalOpen(true)} className="bg-slate-900 hover:bg-slate-800 text-white px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 active:scale-95 shadow-lg shadow-slate-900/20"><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" clipRule="evenodd" /></svg>{t('addFund')}</button>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fadeIn">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            ...
            <p className="text-3xl font-bold text-slate-900 mt-2 tracking-tight">{funds.length > 0 && funds[0].currency ? funds[0].currency : '$'} {totalMarketValue.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
          </div>
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t('avgRisk')}</p>
            <div className="flex items-center gap-3 mt-2"><p className="text-3xl font-bold text-slate-900 tracking-tight">{averageRisk}</p><div className="h-2.5 w-24 bg-slate-100 rounded-full overflow-hidden shadow-inner"><div className={`h-full transition-all duration-1000 ease-out ${Number(averageRisk) > 5 ? 'bg-red-500' : Number(averageRisk) > 3 ? 'bg-yellow-500' : 'bg-emerald-500'}`} style={{ width: `${(Number(averageRisk) / 7) * 100}%` }}></div></div></div>
          </div>
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t('trackedFunds')}</p>
            <p className="text-3xl font-bold text-slate-900 mt-2 tracking-tight">{funds.length}</p>
          </div>
        </div>

        <AnalysisCard analysis={analysis} loading={analyzing} onAnalyze={runAnalysis} hasFunds={funds.length > 0} />

        <div className="mt-10">
          <div className="flex flex-col sm:flex-row justify-between items-end sm:items-center mb-6 gap-4">
             <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">{t('myWatchlist')}<span className="text-xs font-medium text-slate-500 bg-slate-200/50 px-2.5 py-0.5 rounded-full">{funds.length} items</span></h2>
             {funds.length > 0 && (
               <div className="flex items-center gap-3">
                 {selectedFunds.size > 0 && (<><span className="text-sm font-bold text-slate-700 bg-slate-100 px-3 py-1.5 rounded-lg">{t('selected', { count: selectedFunds.size })}</span><button onClick={handleBatchCompare} className="p-2 text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg" title={t('compare')}><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M5 4a1 1 0 00-2 0v7.268a2 2 0 000 3.464V16a1 1 0 102 0v-1.268a2 2 0 000-3.464V4zM11 4a1 1 0 10-2 0v1.268a2 2 0 000 3.464V16a1 1 0 102 0V8.732a2 2 0 000-3.464V4zM16 3a1 1 0 011 1v7.268a2 2 0 010 3.464V16a1 1 0 11-2 0v-1.268a2 2 0 010-3.464V4a1 1 0 011-1z" /></svg></button><button onClick={handleBatchRemove} className="p-2 text-red-600 bg-red-50 hover:bg-red-100 rounded-lg" title={t('batchRemove')}><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clipRule="evenodd" /></svg></button></>)}
                 <button onClick={() => setShowFavoritesOnly(!showFavoritesOnly)} className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all ${showFavoritesOnly ? 'bg-yellow-100 text-yellow-700 border border-yellow-200' : 'bg-white text-slate-600 border border-slate-200'}`}><svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 ${showFavoritesOnly ? 'fill-current' : 'text-slate-400'}`} fill={showFavoritesOnly ? "currentColor" : "none"} viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" /></svg>{showFavoritesOnly ? t('showAll') : t('favoritesOnly')}</button>
               </div>
             )}
          </div>

          {displayedFunds.length === 0 ? (
            <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-slate-300"><h3 className="text-lg font-bold text-slate-700">{showFavoritesOnly ? t('noFavorites') : t('noFunds')}</h3><p className="text-slate-500 mt-2">{showFavoritesOnly ? t('noFavoritesDesc') : t('noFundsDesc')}</p></div>
          ) : (
      ...
      <AddFundModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onAdd={handleAddFund} />
